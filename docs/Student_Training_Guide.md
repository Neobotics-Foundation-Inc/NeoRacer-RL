...
Describes the average points the car earns per lap. We want a graph that climbs up steadily.
...
Describes ow many physics steps it takes the car to finish (or crash). Once the AI stops crashing and starts finishing laps, you want this graph to trend downward. Fewer steps mean a faster lap time!

**3. Policy Loss (The Brain Health)**

Describes the magnitude of how much the neural network is changing its internal weights. A flat, stable, micro-range line (e.g., oscillating between `0.01` and `0.03`). If this graph spikes massively or looks like a violent earthquake, the reward math is broken and the AI is panicking.
